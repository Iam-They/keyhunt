                ...
                fixed (byte* buffer = _buffer) {
                    fixed (uint* blockDWords = _blockDWords) {
                        if ((bufferLen > 0) && (bufferLen + partInLen >= 64)) {
                            BufferX.InternalBlockCopy(partIn, partInBase, _buffer, bufferLen, 64 - bufferLen);
                        ...
                        while (partInLen >= 64) {
                            BufferX.InternalBlockCopy(partIn, partInBase, _buffer, 0, 64);
                            partInBase += 64;
                            partInLen -= 64;
                            MDTransform(blockDWords, stateMD160, buffer);
                        }
                        ...
                        if (partInLen > 0) {
                            BufferX.InternalBlockCopy(partIn, partInBase, _buffer, bufferLen, partInLen);
...
Process {
    ...
    $RIPEMD160 = New-Object -TypeName 'KeyHunter.RIPEMD160'
    ...
    try {
        for(;;) {
            ...
            $temp = $RIPEMD160.ComputeHash($bytes)

            [System.Convert]::ToBase64String($temp) | Write-Output
            }
    }
    finally {
        $reader.Close()
